...
/*function savePublication(req,res){
    ...
    publication.save((err, publicationStored)=>{
      ...
      if(!publicationStored) return res.status(404).send({message:'la publicaci�n no ha sido guardada'});
...
/*async function savePublication(req, res) {
    try {
      ...
      console.error(err);
      return res.status(500).send({ message: 'Error al guardar la publicaci�n' });
    }
  }*/
...
/*
  ...
  function getPublication(req, res) {
    ...
    Publication.findById(publicationId)
      ...
      .then((publication) => {
        if (!publication) {
          return res.status(404).send({ message: 'No existe la publicaci�n' });
        }
        return res.status(200).send({ publication });
      })
      .catch((err) => {
 ...
 /* async function uploadImage(req, res) {
    ...
    if (req.files) {
      ...
      if (file_ext == 'png' || file_ext == 'jpg' || file_ext == 'jpeg' || file_ext == 'gif') {
        ...
        try {
          // Actualizar documento de la publicaci�n
          ...
          if (!publicationUpdated) {
            return res.status(404).send({ message: 'No se ha podido actualizar la publicaci�n' });
          }
        ...
        removeFilesOfUploads(res, file_path, 'Extensi�n no v�lida');
      }
    } else {
      return res.status(200).send({ message: 'No se ha subido imagen' });
    }
